  ...
  "consensus": {
    ...
    def classify_intent(self, query: str) -> IntentClassification:
        ...
        try:
            ...
            # No fallbacks - raise exception for production readiness
            raise RuntimeError(f"Intent analysis failed after {exec_time:.2f}s: {str(e)}")

# ------------------------ MAIN INTENT TOOL CLASS ------------------------

class DcisionAI_Intent_Tool:
    """
    Main DcisionAI Intent Tool - Production-ready intent classification.
    Uses the latest v6 implementation internally.
    """
    
    def __init__(self):
        self.v6_tool = DcisionAI_Intent_Tool_v6()
        self.logger = logging.getLogger(f"{__name__}.DcisionAI_Intent_Tool")
        self.logger.info("✅ DcisionAI Intent Tool initialized")
    
    def classify_intent(self, query: str) -> IntentClassification:
        """Classify intent using the latest implementation"""
        return self.v6_tool.classify_intent(query)
...
def create_intent_tool() -> DcisionAI_Intent_Tool:
    """Create intent tool with standard interface"""
